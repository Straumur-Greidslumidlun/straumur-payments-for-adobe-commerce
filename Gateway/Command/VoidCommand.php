...
class VoidCommand implements CommandInterface
{
    ...
    {
        $this->logger->info('Executing void command', ['subject' => $commandSubject]);
        ...
        try {
            ...
            if (!$payment->getParentTransactionId() && !$payment->getLastTransId()) {
                throw new LocalizedException(__('Cannot void payment without authorization'));
            }
            ...
            if ($this->validator) {
                ...
                if (!$result->isValid()) {
                    ...
                    throw new LocalizedException(__('Void validation failed: %1', $errorMessages));
                }
            }

            $this->responseHandler->handle($commandSubject, $response);
            
            $this->logger->info('Void command completed successfully');
            
            return $response;
            
        } catch (\Exception $e) {
            $this->logger->error('Void command failed', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            
            throw new LocalizedException(__('Void failed: %1', $e->getMessage()), $e);
        }
    }
}
